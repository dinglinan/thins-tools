...
JH.mod.add(['jsonH.nav', 'listenResizeWin', 'ad', 'lang'], 'jsonH', function (modName, JH, $$) {
	...
	var _pub_static = function (sJson) {
		var _checkArgs, _parseDOM, _init, _uiEvt, _custEvt, _airEvt, _main, _this = this, _args = arguments, _pri = {}, _pro = {}, _pub = {__varyContext_:function (pro, pub) {debugger;_pro = pro;_pub = pub;}}, _mod, _base, _parent;
		...
		JH.mergePropertyFrom(_pri, {
			...
			"getAdData" : function (cb) {
				_pri.request.create(JH.request.NS.jsonH, 'getAdData', {
					succeed : function (oAdData) {
						cb(oAdData);
					}
				}).send({
					url : 'http://jsonhandle-addondownload.stor.sinaapp.com/jh.json'
				});
			},
			...
			"showAdTips" : function () {
				$(_pri.node.rcmd).addClass('tips').empty().append('<a href="/options.html#ad" target="_blank" class="adHelp">？关于推广</a>');
			},
			"showAd" : function (oAd) {
				$(_pri.node.rcmd).append('<a href="'+oAd.url+'" target="_blank" class="rcmdContent" data-id="'+oAd.id+'" title="'+oAd.title+'"><img src="'+oAd.pic+'" /></a><button id="closeAd">☓</button>');
			},
