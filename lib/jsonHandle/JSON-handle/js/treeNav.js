	...
	var _pub_static = function (sId) {
		...
		JH.mergePropertyFrom(_pri, {
			...
			"drawElm" : function (oData, sKey, targetBox) {//debugger;
				...
				var sHTML = '<div class="row"><img class="ico" src="css/treePic/' + _pro.icoConfig[sType].icoName + '" alt="" /><div class="elmBox"><span class="elmSpan"><span class="elm '+sElmKeyType+'" title="' + sValue + '">' + sKey + showID + '</span></span></div></div>';
				...
				if(_pub.showAllImg) {
					_pri.drawElmCallback(elmBlock, 1);
			...
			"renderValueImg" : function () {
				...
				$('.elmBlock').each(function () {
					if(!$('.elmBlock', this).length) {
						$('.value', this).remove();
						_pri.drawElmCallback(this, true);
