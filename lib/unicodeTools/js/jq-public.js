...
function Init() {
    ...
    var inputSave = {
        ...
        FillUrls: function (cookieName) {
            ...
            var html = "<li><a href='javascript:;' class='add'>＋保存输入框的网址</a></li>";
            if (iswords) {
        ...
        ToolBoxAdd: function () {
            ...
            if (location.host.indexOf("mobile") >= 0)//如果是mobile.chinaz.com
                $.ajax({ type: "POST", url: "/fit/toobox", data: { "addval": escape(val), "iswords": iswords} });
            else
                $.ajax({ type: "POST", url: "/ajaxsync.aspx", data: 'at=toolbox&iswords=' + iswords + '&addval=' + escape(val) });
        },
        ...
        ToolBoxDeleteValue: function (val) {
            inputSave.BoxHide();
            if (location.host.indexOf("mobile") >= 0)//如果是mobile.chinaz.com
                $.ajax({ type: "POST", url: "/fit/toobox", data: { "delval": escape(val), "iswords": iswords} });
            else
                $.ajax({ type: "POST", url: "/ajaxsync.aspx", data: 'at=toolbox&iswords=' + iswords + '&delval=' + escape(val) });
        },
